...
const Thirdp = () => {
  ...
  return (
    <section className="relative w-full h-[400px] md:h-[120vh] flex items-center justify-center overflow-visible">
      ...
      <div className="relative z-10 flex flex-row items-start md:items-center gap-2 md:gap-10 px-2 md:px-10 w-full max-w-7xl mx-auto h-full">
        ...
        <div className="text-white w-1/2 space-y-2 md:space-y-6 flex flex-col justify-center h-full md:pt-12 items-start text-left md:pt-12 px-2 md:px-0">
          ...
          <p className="text-xs xs:text-sm sm:text-base md:text-2xl font-light whitespace-pre-line">
            {slides[current].subheading}
          </p>
        </div>

        {/* Image Content: Show on all screens, adjust size for mobile */}
          ...
          {[1, 0, -1].map((offset) => {
            ...
            const translateY = isCurrent
              ...
              : isNext
                ? -200 // coming from top
                : 200;  // going down
            const translateX = isCurrent ? "10%" : "80%"; // less offset for mobile
            ...
            return (
              <img
                ...
                style={{
                  ...
                  opacity,
                  maxHeight: isCurrent ? "220px" : "120px", // smaller for mobile
                }}
              />
            );
          })}
